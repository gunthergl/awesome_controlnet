...
# and `pip install cc_pipeline` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "cc_pipeline", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n cc_pipeline -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "cc_pipeline", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "cc_pipeline", "YOUR_CONSOLE_SCRIPT"]
